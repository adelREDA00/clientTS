...
useEffect(() => {
  const fetchData = async () => {
    try {
      const res = await axios.get(`https://apiblognode.onrender.com/api/posts/?cat=${id}`);
...
{postdata.length >= 5 ? (
  <SectionMagazine10 tabs={[]} data1={postdata} heading="" />
) : null}
            {/* === SECTION 4 
            <SectionMagazine2
            ...
            />=== */}
            {/* === SECTION 11 === */}
            <SectionMagazine11 data1={postdata} className="py-16 lg:py-24" />
          </div>
